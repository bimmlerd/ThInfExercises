...
\intertext{Damit eine Formel erfüllbar ist, ist sie nicht widersprüchlich ( \( \nexists x_i\ s.t.\ x_i = 1, \overline{x_i} = 1 \) ) und muss man in jeder Klausel eine Variabel für die Erfüllung der Klausel verantwortlich machen können.}
...
%Die Idee dabei ist, auszunutzen, dass wenn \( x \in \) SUBSET-SUM, folgt, dass es $x$ die Kodierung einer Menge $S$ von natürlichen Zahlen und einer natürlichen Zahl ist, wobei eine Untermenge $U$ von $S$ existiert, deren Elemente zu $t$ summieren.
Wir nutzen aus, dass aus \( x \in \) SUBSET-SUM folgt, dass $x$ die Kodierung einer Menge $S$ von natürlichen Zahlen und einer natürlichen Zahl $t$ ist, wobei eine Untermenge $U$ von $S$ existiert, deren Elemente zu $t$ summieren. Wir bemerken, dass dies nicht allzu weit von der Bedingung, die von PARTITION gestellt wird, entfernt ist. Wir müssen also sicherstellen, dass es einen ``Gegenpart'' zu $U$ gibt, der ebenfalls zu $t$ aufsummiert, jedoch kein Element aus $U$ enthalten kann. \\
