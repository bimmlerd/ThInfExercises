...
Des weiteren folgt, dass \(M\) ein Wort \(w \in L(M) \) innerhalb von \(c'  \cdot f(n)^k \) Schritten akzeptieren muss, für ein bestimmtes  \(c\). \\
...
Insofern nicht anders erwähnt, funktionniert die angestrebte NMTM gleich als in Satz 6.7. \\
...
Insgesamt ergiebt dies also \( \Oh(f(n)) \cdot \Oh(\log(f(n)))= \Oh(f(n) \cdot \log(f(n))) \) viel Speicherplatz.
\end{enumerate}
\end{homeworkProblem}
