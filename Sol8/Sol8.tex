...
\begin{enumerate}[a)]
\item Zu beweisen ist die folgende Behauptung: \[ L_1, L_2 \in \NTIME(f) \implies L_1 \cup L_2 \in \NTIME(f)\]  $ L_1, L_2 \in \NTIME(f) \implies \exists M_1, M_2$ so dass $\Time_{M_1}$, $\Time_{M_2} \in \Oh(f(n))$ mit $L(M_1) = L_1$, $L(M_2) = L_2$. Wir definieren die Sprache $L = L_1 \cup L_2$ und eine NTM $M$ mit $L(M) = L$. $M$ muss also genau dann akzeptieren, wenn $M_1$ oder $M_2$ akzeptieren. Wir konstruieren $M$ aus $M_1$ und $M_2$, indem wir die beiden parallel simulieren. $M$ akzeptiert sobald $M_1$ oder $M_2$ akzeptieren und verwirft falls sowohl $M_1$ wie auch $M_2$ verwerfen.
